...
bool TaprootAddress::isValid(const std::string& string) {
    return std::get<2>(decode(string));
}

bool TaprootAddress::isValid(const std::string& string, const std::string& hrp) {
...
TaprootAddress::TaprootAddress(const PublicKey& publicKey, std::string hrp)
    : hrp(std::move(hrp)), witnessVersion(0), witnessProgram() {
    if (publicKey.type != TWPublicKeyTypeSECP256k1) {
        throw std::invalid_argument("TaprootAddress needs a compressed SECP256k1 public key.");
    }
...
std::tuple<TaprootAddress, std::string, bool> TaprootAddress::decode(const std::string& addr) {
    auto resp = std::make_tuple(TaprootAddress(), "", false);
...
std::string TaprootAddress::string() const {
    Data enc;
    enc.push_back(witnessVersion);
    Bech32::convertBits<8, 5, true>(enc, witnessProgram);
    Bech32::ChecksumVariant variant = Bech32::ChecksumVariant::Bech32M;
    if (witnessVersion == 0) {
...
std::pair<TaprootAddress, bool> TaprootAddress::fromRaw(const std::string& hrp, const Data& data) {
    auto resp = std::make_pair(TaprootAddress(), false);
    if (data.size() == 0) {
        return resp;
    }
    ...
    return std::make_pair(TaprootAddress(hrp, data[0], conv), true);
}
